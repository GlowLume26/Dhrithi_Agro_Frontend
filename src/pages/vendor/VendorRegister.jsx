...
const SELLER_DOCS = [
  { id: 'aadhaar',  icon: '🪪', label: 'Aadhaar Card',         sub: 'Front & Back' },
  { id: 'pan',      icon: '💳', label: 'PAN Card',              sub: 'Business or Personal PAN' },
  { id: 'gst_doc',  icon: '📋', label: 'GST Certificate',       sub: 'GST Registration Certificate' },
  { id: 'licence',  icon: '📜', label: 'Trade / Shop Licence',  sub: 'Shop Act / Trade Licence' },
...
export default function VendorRegister() {
  ...
  const isSeller = searchParams.get('role') !== 'buyer';
  const UPLOAD_DOCS = isSeller ? SELLER_DOCS : BUYER_DOCS;
  ...
  function handleUpload(id, file) { if (file) setUploads(u => ({ ...u, [id]: { name: file.name, file } })); }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      fd.append('city',           biz.bizCity);
      fd.append('state',          biz.bizState);
      fd.append('pincode',        biz.bizPin);
      if (biz.bizEmail)    fd.append('email',           biz.bizEmail);
      if (biz.panNum)      fd.append('pan_number',      biz.panNum);
      if (isSeller) {
        fd.append('business_name', biz.bizName);
        if (biz.gstNum) fd.append('gst_number', biz.gstNum);
      }
  ...
  const stepLabels = isSeller
    ? ['Business Details', 'Documents', 'Store Setup', 'Review']
    : ['Personal Details', 'Documents', 'Review'];
  ...
  if (success) return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: 40, textAlign: 'center' }}>
      ...
      <p style={{ fontSize: 15, color: '#666', lineHeight: 1.7, maxWidth: 500, margin: '0 auto 24px' }}>
        Your {isSeller ? 'seller' : 'buyer'} registration has been submitted. Our team will review your documents and get back to you within <b>2–3 business days</b>.
      </p>
    ...
    <>
      ...
      <div className="vendor-reg-page">
        ...
        <div className="vendor-hero">
          <h1>{isSeller ? '🏪 Sell on Drithi Agro' : '🛒 Buy on Drithi Agro'}</h1>
          <p>{isSeller
            ? "Join India's fastest growing agri-marketplace. Reach 5 lakh+ farmers across 28 states."
            : 'Register as a buyer and get access to quality agri products at the best prices.'}</p>
          <div className="vendor-benefits">
            {isSeller
              ...
              : ['🚚 Free Delivery','✅ Genuine Products','💰 Best Prices','🧑‍🌾 Expert Support'].map(b => <span key={b} className="vb">{b}</span>)
            }
          </div>
        </div>
        ...
        <div className="reg-steps">
          {stepLabels.map((l, i) => (
            <span key={l} style={{ display: 'contents' }}>
              {i > 0 && <div className={'rs-line' + (step > i ? ' done' : '')} />}
              <div className={`rs${step === i + 1 ? ' active' : step > i + 1 ? ' done' : ''}`}>
          ...
          <div className="form-section">
            ...
            <div className="form-grid">
              {(isSeller
                ...
                : [['Full Name','ownerName','text'],['Mobile Number','bizMobile','tel'],['Email Address','bizEmail','email'],['PAN Number','panNum','text']]
              ).map(([l,k,t]) => (
              ...
              <div className="form-group full"><label>{isSeller ? 'Business' : 'Delivery'} Address <span style={{color:'#e53935'}}>*</span></label><textarea rows={2} value={biz.bizAddr} onChange={e => setBiz(b => ({...b,bizAddr:e.target.value}))} /></div>
        ...
        {step === 2 && (
          <div className="form-section">
            ...
            <p style={{ fontSize: 13, color: '#666', marginBottom: 20 }}>
              Upload clear scanned copies or photos. Accepted formats: JPG, PNG, PDF (Max 5MB each)
              {isSeller && <span style={{ color:'#e53935', fontWeight:600 }}> — GST Certificate & Trade Licence required for sellers</span>}
            </p>
            <div className="upload-grid">
              {UPLOAD_DOCS.map(doc => (
                <label key={doc.id} className={uploads[doc.id] ? 'doc-uploaded' : 'upload-box'} style={{ cursor: 'pointer' }}>
                  ...
                  <h4>{doc.label}</h4>
                  {uploads[doc.id] ? <span>✅ {uploads[doc.id].name}</span> : <p>{doc.sub}</p>}
                </label>
              ))}
            </div>
            <div style={{ display: 'flex', gap: 12, justifyContent: 'space-between', marginTop: 16 }}>
              ...
              <button onClick={() => goStep(isSeller ? 3 : 4)} style={{ background: '#2e7d32', color: 'white', border: 'none', padding: '14px 32px', borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
                {isSeller ? 'Next: Store Setup →' : 'Review & Submit →'}
              </button>
            </div>
          </div>
        )}

        {/* STEP 3 — Store Setup (Seller only) */}
        {isSeller && step === 3 && (
          <div className="form-section">
            <h3>🏪 Store Details</h3>
            <div className="form-grid">
        ...
        {step === 4 && (
          <div className="form-section">
            ...
            <div style={{ background: '#f9fbe7', borderRadius: 12, padding: 20, marginBottom: 16 }}>
              <h4 style={{ fontSize: 15, fontWeight: 700, color: '#1b5e20', marginBottom: 12 }}>{isSeller ? 'Business' : 'Personal'} Details</h4>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, fontSize: 13 }}>
                {(isSeller
                  ? [['Business Name', biz.bizName], ['Owner Name', biz.ownerName], ['Mobile', biz.bizMobile], ['Email', biz.bizEmail], ['GST Number', biz.gstNum], ['PAN Number', biz.panNum]]
                  : [['Full Name', biz.ownerName], ['Mobile', biz.bizMobile], ['Email', biz.bizEmail], ['PAN Number', biz.panNum]]
                ).map(([k, v]) => (
            ...
            <div style={{ background: '#e8f5e9', borderRadius: 12, padding: 16, marginBottom: 16, fontSize: 13, color: '#2e7d32' }}>
              ...
              3. Once approved, your {isSeller ? 'Vendor ID and login credentials' : 'Buyer account'} will be activated.
            </div>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 16 }}>
              ...
              <label style={{ fontSize: 13, color: '#555', cursor: 'pointer' }}>
                I agree to the <Link to="/terms" target="_blank" style={{ color: '#2e7d32', fontWeight: 600 }}>Terms & Conditions</Link> and{' '}
                <Link to="/terms" target="_blank" style={{ color: '#2e7d32', fontWeight: 600 }}>{isSeller ? 'Seller Policy' : 'Buyer Policy'}</Link> of Drithi Agro Marketplace.
              </label>
            </div>
            {submitErr && <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#dc2626', marginBottom: 12 }}>⚠️ {submitErr}</div>}
            <div style={{ display: 'flex', gap: 12, justifyContent: 'space-between' }}>
              <button onClick={() => goStep(isSeller ? 3 : 2)} style={{ background: '#f5f5f5', color: '#333', border: 'none', padding: '14px 32px', borderRadius: 12, fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>← Back</button>
              <button onClick={handleSubmit} disabled={submitting} style={{ background: '#2e7d32', color: 'white', border: 'none', padding: '14px 40px', borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.75 : 1 }}>
